...
class PostgreSQLAdapter(BaseDialectAdapter):
    def discover_schemas(self, connection_url: str) -> list[SchemaMeta]:
        engine = create_engine(connection_url)
        inspector = inspect(engine)

        system_schemas = {"pg_catalog", "information_schema", "pg_toast"}
        schemas = [s for s in inspector.get_schema_names() if s not in system_schemas]

        result_schemas: list[SchemaMeta] = []
        ...
        for schema in schemas:
            ...
            result_schemas.append(SchemaMeta(schema_name=schema, tables=tables))
